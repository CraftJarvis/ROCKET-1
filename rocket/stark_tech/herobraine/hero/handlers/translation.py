...
import numpy as np
from rocket.stark_tech.herobraine.hero.spaces import MineRLSpace
import rocket.stark_tech.herobraine.hero.spaces as spaces
        ...
        for key in keys:
            if key in d:
                ...
                if self.default_if_missing is not None:
                    # self.logger.error(f"{self.to_string()}: No {keys[-1]} observation! Yielding default value "
