...
import rocket.arm.utils.vpt_lib.xf as xf
from rocket.arm.utils.vpt_lib.minecraft_util import store_args
from rocket.arm.utils.vpt_lib.tree_util import tree_map
from typing import Tuple

@functools.lru_cache()
def get_diagonal_true_mask(b: int, t: int, T: int, device: th.device):
    upper = th.triu(th.ones((t, T)), diagonal=T-t)
    lower = th.tril(th.ones((t, T)), diagonal=T-t)
    mask = (upper + lower - 1).bool().to(device=device)
    return mask[None].repeat_interleave(b, dim=0)
...
def get_mask(first_b1t: th.Tensor, state_mask: th.Tensor, t: int, T: int, maxlen: int, heads: int, device, word_mask=None) -> Tuple[th.Tensor, th.Tensor]:
    ...
    if word_mask is not None:
        m_btT = (m_btT & word_mask) | get_diagonal_true_mask(b, t, T, device=device)
        # print(0, m_btT[0, 0, 124:134])
        # print(1, m_btT[0, 1, 124:134])
        # print(2, m_btT[0, 2, 124:134])
        # print(3, m_btT[0, 3, 124:134])
    ...
    def forward(self, input_bte, first_bt, state, **kwargs):
        ...
        if self.mask == "clipped_causal":
            new_mask, state_mask = get_mask(
                ...
                word_mask=kwargs.get('word_mask', None),
            )
