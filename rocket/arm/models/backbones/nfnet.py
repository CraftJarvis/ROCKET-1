...
class NFNet(nn.Module):
    def __init__(self, num_classes:int, variant:str='F0', stochdepth_rate:float=0.25, 
...
class CustomNFNet(nn.Module):
    ...
    def __init__(
        ...
        **kwargs, 
        ...
        super().__init__()
        self.model = NFNet(variant=version, num_classes=out_dim)
...
if __name__ == '__main__':
    # model = NFNet(variant='F0', num_classes=1024, stochdepth_rate=0.25)
    model = CustomNFNet(version='F0', out_dim=1024, pooling=False)
    R = 128
    x = torch.rand(8, 5, 3, R, R)
    y = model(x)
    print(y.shape)